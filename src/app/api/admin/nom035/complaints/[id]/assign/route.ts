...
import { assignComplaint, complaintAssignSchema } from "@/lib/nom035/server/complaint-service";
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const parsed = complaintAssignSchema.safeParse(body.value);
    if (!parsed.success) return adminJsonError("invalid_payload", requestId);
    const data = await assignComplaint(id, parsed.data.assignedLabel);
