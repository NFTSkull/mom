...
import { closeComplaint, complaintCloseSchema } from "@/lib/nom035/server/complaint-service";
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const parsed = complaintCloseSchema.safeParse(body.value);
    if (!parsed.success) return adminJsonError("invalid_payload", requestId);
    const data = await closeComplaint(id, parsed.data);
