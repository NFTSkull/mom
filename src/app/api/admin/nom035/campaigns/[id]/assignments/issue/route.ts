...
import { issueAssignment } from "@/lib/nom035/server/admin-campaign-service";

export const runtime = "nodejs";

type Ctx = { params: Promise<{ id: string }> };

const bodySchema = z.object({ workerId: z.string().uuid() }).strict();
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const data = await issueAssignment({
      campaignId: id,
